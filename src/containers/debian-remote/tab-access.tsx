...
export const RemoteAccessTab = ({
  ...
  const query = () => {
    ...
    DebianRemoteAPI.listRoles(id, { page_size: 100 })
      .then(({ data: { roles } }) => {
  ...
  const removeUserRole = (role, user) => {
    const removedRole = DebianRemoteAPI.removeRole(id, {
      role,
      users: [user.username],
    });
    updateRoles({
  ...
  return (
    <AccessTab
      ...
      groups={groups}
      name={name}
      pulpObjectType='remotes/debian/collection'
      removeGroup={removeGroup}
      removeRole={removeRole}
      removeUser={removeUser}
      removeUserRole={removeUserRole}
      selectRolesMessage={t`The selected roles will be added to this specific Debian remote.`}
      ...
      user={selectedUser}
      users={users}
      urlPrefix={formatPath(Paths.debian.remote.detail, {
        name,
      })}
    />
  );
