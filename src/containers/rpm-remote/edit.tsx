...
const initialRemote: RPMRemoteType = {
  name: '',
  url: '',
  ca_cert: null,
  client_cert: null,
...
const RPMRemoteEdit = Page<RPMRemoteType>({
  breadcrumbs: ({ name }) =>
    [
      { url: formatPath(Paths.rpm.remote.list), name: t`Remotes` },
      name && { url: formatPath(Paths.rpm.remote.detail, { name }), name },
      name ? { name: t`Edit` } : { name: t`Add` },
    ].filter(Boolean),

  displayName: 'RPMRemoteEdit',
  errorTitle: msg`Remote could not be displayed.`,
  listUrl: formatPath(Paths.rpm.remote.list),
  query: ({ name }) =>
    RPMRemoteAPI.list({ name }).then(({ data: { results } }) => results[0]),
  ...
  render: (item, { navigate, queueAlert, state, setState }) => {
    ...
    const saveRemote = () => {
      ...
      const promise = !item
        ? RPMRemoteAPI.create(data)
        : RPMRemoteAPI.smartUpdate(
      ...
      promise
        .then(({ data: task }) => {
          ...
          navigate(
            formatPath(Paths.rpm.remote.detail, {
              name: data.name,
            }),
          );
        })
    ...
    const closeModal = () => {
      ...
      navigate(
        item
          ...
          : formatPath(Paths.rpm.remote.list),
      );
    };
