...
export const RepositoryAccessTab = ({
  ...
  const query = () => {
    ...
    DebianRepositoryAPI.listRoles(id, { page_size: 100 })
      .then(({ data: { roles } }) => {
  ...
  const addUser = (user, roles) => {
    const rolePromises = roles.map((role) =>
      DebianRepositoryAPI.addRole(id, {
        role: role.name,
        users: [user.username],
      }),
    );
  ...
  const removeUser = (user) => {
    const roles = user.object_roles.map((role) =>
      DebianRepositoryAPI.removeRole(id, {
        role,
        users: [user.username],
      }),
    );
  ...
  const removeUserRole = (role, user) => {
    const removedRole = DebianRepositoryAPI.removeRole(id, {
      role,
      users: [user.username],
    });
    updateRoles({
  ...
  return (
    <AccessTab
      ...
      groups={groups}
      name={name}
      pulpObjectType='repositories/debian/debian'
      removeGroup={removeGroup}
      removeRole={removeRole}
      removeUser={removeUser}
      removeUserRole={removeUserRole}
      selectRolesMessage={t`The selected roles will be added to this specific Debian repository.`}
      ...
      user={selectedUser}
      users={users}
      urlPrefix={formatPath(Paths.debian.repository.detail, {
        name,
      })}
    />
  );
