...
import { RPMDistributionAPI, RPMRepositoryAPI } from 'src/api';
...
export const rpmRepositoryDeleteAction = Action({
  title: msg`Delete`,
...
async function deleteRepository(
  ...
  const distributionsToDelete = await RPMDistributionAPI.list({
  ...
  const deleteRepo = RPMRepositoryAPI.delete(pulpId)
    .then(({ data }) => {
  ...
  const deleteDistribution = ({ name, pulp_href }) => {
    ...
    return RPMDistributionAPI.delete(distribution_id)
      .then(({ data }) => {
        addAlert(
